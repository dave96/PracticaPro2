...
public:
    ...
    \pre cert
    \post be = cert si s'ha llegit una comanda sense errors sintactics; altrament, be = fals i s'escriu "Error de format"; retorna cert quan la comanda no es "sortir" */
    bool llegir(bool& be);
    ...
    /** @brief Comprova si la comanda es una modificacio (el primer string es un enter, pero el segon no comen̤a per "-")
    \pre cert
    ...
    /** @brief Comprova si la comanda es d'esborrat (comen̤a per "-k" o per "k -x", on k es un enter)
    \pre cert
    ...
    \pre 1 <= i <= n, on n es el nombre d'etiquetes per afegir que conte la comanda
